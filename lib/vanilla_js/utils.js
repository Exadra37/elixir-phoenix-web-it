...
const removeClassById = function(id, className) {
  let el = document.getElementById(id)
  removeClassFromElement(el, className)
}

const removeClassFromElement = function(el, className) {
  if (el && el.classList) {
    el.classList.remove(className)
  }
}

const addClassTo = function(id, className) {
  let el = document.getElementById(id)
...
const animatedFadeInById = function(id, fade_in_class, timeout) {
  addClassTo(id, fade_in_class)
  displayElement(id)

  // Give time for the fade in animation to run.
  setTimeout(function () {
    removeClassById(id, fade_in_class)
  }, timeout);
}

const animatedFadeOutById = function(id, fade_out_class, timeout) {
  addClassTo(id, fade_out_class)

  // Give time for the fade out animation to run.
  setTimeout(function () {
    removeClassById(id, fade_out_class)
    hideElement(id)
  }, timeout);
}

const updateValueForId = function(id, value) {
  const el = document.getElementById(id)
